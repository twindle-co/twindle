...
function getURL(url_type) {
	var url = "";
	if (url_type == "tweet-lookup") url += ENDPOINT_TO_FETCH_CONVERSATION_ID + tweet_id;
...
function processSearchResponse(responseJSON) {
	var directReplies = getTweetArray(responseJSON).filter(
		tweet => tweet.referenced_tweets.filter(ref => ref.type == "replied_to" && ref.id == tweet_id).length > 0
	);
