...
const USER_TIMELINE_ENDPOINT =
	"https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=<screen_name>&since_id=<tweet_id>&tweet_mode=extended&count=200";
...
function getURL(url_type) {
	var url = "";
	if (url_type == "tweet-lookup") url = TWEET_LOOKUP_ENDPOINT.replace("<tweet_id>", tweet_id);
...
function processResponse(url_type, responseJSON) {
	if (url_type == "tweet-lookup") processTweetLookup(responseJSON);
	else if (url_type == "search-tweets") processSearchResponse(responseJSON);
}
...
function processSearchResponse(responseJSON) {
	var directReply = getTweetArray(responseJSON).filter(
		tweet => tweet.in_reply_to_screen_name == screen_name && tweet.in_reply_to_status_id_str == tweet_id
	);
	while (directReply.length > 0) {
		var tweet = directReply[0];
		var replyId = tweet.id_str;
		createCustomTweet(tweet, tweet.user);
		directReply = getTweetArray(responseJSON).filter(
			tweet => tweet.in_reply_to_screen_name == screen_name && tweet.in_reply_to_status_id_str == replyId
		);
	}
...
async function fetchURL(url, url_type) {
	...
	await fetch(url, requestOptions)
		...
		.catch(error => console.devLog("error", error));
}
