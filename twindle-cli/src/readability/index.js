...
function getArticleJSON(article, url, windowDocument) {
    ...
    if(article.byline)
        articleJSON.author = article.byline;
    let imageTag = Array.from(windowDocument.head.getElementsByTagName("meta"))
                        .filter(element => element.getAttribute('property') === 'og:image')[0];
    if(imageTag)
        articleJSON.image = imageTag.getAttribute("content");
    return articleJSON;
}
